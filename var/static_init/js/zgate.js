...
var use2dir = new Object({'1003:1.2.840.10003.3.1':'',
			 '4:1.2.840.10003.3.1':'25',
			 '21:1.2.840.10003.3.1':'6',
			 '1018:1.2.840.10003.3.1':'5'});
...
function setForm(x) {
  if (unescape(ts).length == 2 &&
	(!(navigator.userAgent.indexOf('Mozilla') >= 0 && rv >= '1.6' || navigator.userAgent.indexOf('Opera') >= 0)) ||
...
function scan(ref, db, attr, attrset, val)
{
var i;
var fl = false;
...
if (s == null || s == '') {
  for (i = 0; i < spans.length; i++ )
    if (spans[i].getAttribute(cl) == 'menu_sr')
...
for (i = 0; i < divs.length; i++ ) {
  if (divs[i].getAttribute(cl) == 'record') {
/*    
    divs[i].setAttribute(cl, 'localrecord');
    divs[i].setAttribute('title', 'Этот документ имеется в Вашей библиотеке');
*/
}
}
}

function checkAttrs(value) {
...
for (i = 0; i < bs.length; i++ ) {
  if (bs[i].getAttribute(cl) == 'dir')
    bs[i].style.display='none';
}

for (i=1; i <=3; i++) {
  a = document.getElementsByName('USE_'+i);
  if (a.length > 0) {
    if (v = use2dir[a[0].value]) {
      x = document.getElementById('dir'+i+v);
