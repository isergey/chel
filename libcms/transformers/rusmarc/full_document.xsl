        ...
        <xsl:element name="div">
            <xsl:choose>
                <xsl:when test="@syntax='1.2.840.10003.5.1' or @syntax='Unimarc'">
                    ...
                    <xsl:if test="$class='record'">
                        <xsl:attribute name="id">
                            <xsl:value-of select="field[@id='001']"/>
                        </xsl:attribute>
                    </xsl:if>
                ...
                <xsl:when test="@syntax='1.2.840.10003.5.102' or @syntax='OPAC'">
                    <xsl:attribute name="class">record</xsl:attribute>
                    <xsl:attribute name="id">
                        <xsl:value-of select="bibliographicRecord/record/field[@id='001']"/>
                    </xsl:attribute>
                ...
                <xsl:when
                        test="@syntax='1.2.840.10003.5.106' or @syntax='Extended' or eSTaskPackage"> <!-- esTaskPackage - temporary workaround -->
                    <xsl:attribute name="class">record</xsl:attribute>
