...
def index(request):
    ...
    if '..' in path or '/.' in path:
        raise Http404("Path not founded")
    ...
    if not chek_or_create_dir(base_uplod_path):
        return HttpResponse("Catalog '%s' can't be created" % show_path)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if os.path.isfile(upload_path + '/' + file_name.encode(FILE_NAME_ENCODING)):
                return HttpResponse(_('File with this name already exist. Please, delete old file or rename uploadable file.'))
            elif  os.path.isdir(upload_path + '/' + file_name.encode(FILE_NAME_ENCODING)):
                return HttpResponse(_('Directory with this name already exist. Please, delete old directory or rename uploadable file.'))
    ...
    if request.method == 'POST':
        ...
        if dir_form.is_valid():
            ...
            if os.path.isfile(upload_path + '/' +dir_name):
                return HttpResponse(_('File with this name already exist. Please, delete old file or rename creatable directory.'))
            elif  os.path.isdir(upload_path + '/' + dir_name):
                return HttpResponse(_('Directory with this name already exist. Please, delete old directory or or rename creatable directory.'))

            if not os.path.isdir(upload_path):
                raise Http404(_("Path not founded"))
            upload_path = upload_path.rstrip('/')
            os.mkdir(upload_path + '/' + dir_name, 0o755)
