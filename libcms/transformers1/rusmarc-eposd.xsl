...
<xsl:template name="subjects">
  ...
  <div class="subjects">
  <xsl:if test="$p600 + $p601 + $p602 + $p605 + $p606 + $p607 + $p610 &gt; 0">
  ...
  <xsl:for-each select="field[@id='606']">
    <xsl:text> </xsl:text>
    <xsl:value-of select="$p600 + $p601 + $p602 + $p605 + position()"/>
    <xsl:text>. </xsl:text>
    <xsl:choose>
      <xsl:when test="$follow.subject">
        <a href="{$cgi.script.URL}?ACTION=follow&amp;SESSION_ID={$session.id}&amp;TERM={subfield[@id='a']}[1,21]&amp;LANG={$lang}">
  ...
  <xsl:for-each select="field[@id='607']">
    <xsl:text> </xsl:text>
    <xsl:value-of select="$p600 + $p601 + $p602 + $p605 + $p606 + position()"/>
    <xsl:text>. </xsl:text>
    <xsl:choose>
      <xsl:when test="$follow.subject">
        <a href="{$cgi.script.URL}?ACTION=follow&amp;SESSION_ID={$session.id}&amp;TERM={subfield[@id='a']}[1,21]&amp;LANG={$lang}">
  ...
  <xsl:for-each select="field[@id='610']">
    <xsl:text> </xsl:text>
    <xsl:value-of select="$p600 + $p601 + $p602 + $p605 + $p606 + $p607 + position()"/>
    <xsl:text>. </xsl:text>
...
<xsl:template match="field[@id='856']">
  <xsl:if test="subfield[@id='u'] and contains(subfield[@id='u'], 'unilib.neva.ru/') and contains(subfield[@id='u'], '.pdf') and not(contains(subfield[@id='u'], '/local/'))">
    <xsl:variable name="u" select="concat('http://epos.unilib.neva.ru/', substring-after(subfield[@id='u'], 'unilib.neva.ru/'))"/>
    <xsl:value-of select="$dlm"/>
    <xsl:choose>
      <xsl:when test="subfield[@id='z']">
        <a href="{$u}"><xsl:value-of select="subfield[@id='z']"/></a>
      </xsl:when>
      <xsl:otherwise>
