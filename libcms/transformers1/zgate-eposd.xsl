...
<xsl:include href="opac-eposd.xsl"/>
<xsl:output
	method="html"
	indent="yes"
	encoding="utf-8"
...
<xsl:template name="record.selector">
  ...
  <xsl:choose>
    ...
    <xsl:when test="@syntax='1.2.840.10003.5.101' or @syntax='OPAC'">
    ...
    <xsl:when test="@syntax='1.2.840.10003.5.106' or @syntax='Extended'">
...
<!-- PQS -->
<!--
    <xsl:if test="(@type='SearchResponse' or @type='PresentResponse') and $user.id">
      <xsl:text> [ </xsl:text>
        <a href="{$cgi.script.URL}?ACTION=prepqs&amp;SESSION_ID={$session.id}&amp;DB={database}&amp;TERM={term}&amp;TARGET={$target}&amp;LANG={$lang}">&#x203d;</a>
      <xsl:text> ] </xsl:text>
    </xsl:if>
-->
<!-- Backwards -->
    <xsl:choose>
      <xsl:when test="@type='SearchResponse' or @type='PresentResponse'">
        <xsl:if test="$start &gt; 1">
          ...
          <xsl:choose>
            <xsl:when test="$fmt = 'B'">
              <a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$bkw.start}+{$fwd.end}+{$fmt}+{$record.syntax}+{$lang}"><xsl:text>&lt;&lt;</xsl:text></a>
            </xsl:when>
            <xsl:otherwise>
              <a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$bkw.start}+1+{$fmt}+{$record.syntax}+{$lang}"><xsl:text>&lt;&lt;</xsl:text></a>
            </xsl:otherwise>
          </xsl:choose>
          <xsl:text> ]</xsl:text>
        </xsl:if>
          ...
          <xsl:text> [ </xsl:text>
            <xsl:choose>
              <xsl:when test="termInfo">
                <a href="{$cgi.script.URL}?ACTION=SCAN&amp;SESSION_ID={$session.id}&amp;DBNAME={database}&amp;THE_TERM={str:encode-uri(termInfo[1]/term, true())}{$attr}&amp;STEP_SIZE={$step.size}&amp;MAXRECORDS={$max.records}&amp;PREF_POS={$max.records}&amp;ATTSET={$attribute.set}&amp;RECSYNTAX={$record.syntax}&amp;LANG={$lang}&amp;CHAR_SET={$neg.cs}"><xsl:text>&lt;&lt;</xsl:text></a>
              </xsl:when>
              <xsl:otherwise>
                <a href="{$cgi.script.URL}?ACTION=SCAN&amp;SESSION_ID={$session.id}&amp;DBNAME={database}&amp;THE_TERM={str:encode-uri(term, true())}&amp;STEP_SIZE={$step.size}&amp;MAXRECORDS={$max.records}&amp;PREF_POS={$max.records}&amp;ATTSET={$attribute.set}&amp;RECSYNTAX={$record.syntax}&amp;LANG={$lang}&amp;CHAR_SET={$neg.cs}"><xsl:text>&lt;&lt;</xsl:text></a>
              </xsl:otherwise>
            </xsl:choose>
          <xsl:text> ]</xsl:text>
        </xsl:if>
    ...
    <xsl:choose>
      <xsl:when test="@type='SearchResponse' or @type='PresentResponse'">
        <xsl:if test="$start + count(record) - 1 &lt; resultCount">
          ...
          <xsl:choose>
            <xsl:when test="$fmt = 'B'">
              <a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$start + count(record)}+{$fwd.end}+{$fmt}+{$record.syntax}+{$lang}"><xsl:text>&gt;&gt;</xsl:text></a>
            </xsl:when>
            <xsl:otherwise>
              <a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$start + count(record)}+1+{$fmt}+{$record.syntax}+{$lang}"><xsl:text>&gt;&gt;</xsl:text></a>
            </xsl:otherwise>
          </xsl:choose>
          <xsl:text> ]</xsl:text>
        </xsl:if>
          ...
          <xsl:text> [ </xsl:text>
            <xsl:choose>
              <xsl:when test="count(termInfo) &gt; 0">
                <a href="{$cgi.script.URL}?ACTION=SCAN&amp;SESSION_ID={$session.id}&amp;DBNAME={database}&amp;THE_TERM={str:encode-uri(termInfo[count(current()/termInfo)]/term, true())}{$attr}&amp;STEP_SIZE={$step.size}&amp;MAXRECORDS={$max.records}&amp;PREF_POS=1&amp;ATTSET={$attribute.set}&amp;RECSYNTAX={$record.syntax}&amp;LANG={$lang}&amp;CHAR_SET={$neg.cs}"><xsl:text>&gt;&gt;</xsl:text></a>
              </xsl:when>
              <xsl:otherwise>
                <a href="{$cgi.script.URL}?ACTION=SCAN&amp;SESSION_ID={$session.id}&amp;DBNAME={database}&amp;THE_TERM={str:encode-uri(term, true())}&amp;STEP_SIZE={$step.size}&amp;MAXRECORDS={$max.records}&amp;PREF_POS={$max.records}&amp;ATTSET={$attribute.set}&amp;RECSYNTAX={$record.syntax}&amp;LANG={$lang}&amp;CHAR_SET={$neg.cs}"><xsl:text>&lt;&lt;</xsl:text></a>
              </xsl:otherwise>
...
<!-- Download all -->
    <xsl:if test="@type='SearchResponse' or @type='PresentResponse'">
      <xsl:if test="$marc.download and $download.all and resultCount &gt; 0">
        <xsl:text>[ </xsl:text><a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+1+{resultCount}+R+{$record.syntax}+{$lang}"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_DOWNLOAD_ALL']"/></a><xsl:text> ]</xsl:text>
      </xsl:if>
    </xsl:if>
...
<!-- Download selected -->
    <xsl:if test="@type='SearchResponse' or @type='PresentResponse'">
      <xsl:if test="$marc.download and $download.selected and resultCount &gt; 0">
        <xsl:text>[ </xsl:text><a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+0+0+R+{$record.syntax}+{$lang}"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_DOWNLOAD_SEL']"/></a><xsl:text> ]</xsl:text>
      </xsl:if>
    </xsl:if>
...
<xsl:template name="record.menu">
  <xsl:if test="$fmt = 'B'">
    <div class="recordmenu">
      ...
      <xsl:text>[ </xsl:text><a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$start + position() - 1}+1+F+{$record.syntax}+{$lang}"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_FULL']"/></a><xsl:text> ]</xsl:text>
    </div>
  </xsl:if>
  <xsl:if test="$fmt = 'F'">
    <div class="recordmenu">
...
<!-- MARC TAGS -->
        <xsl:if test="$marc.labels">
          <xsl:text>[ </xsl:text><a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$start + position() - 1}+1+M+{$record.syntax}+{$lang}"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_TAG']"/></a><xsl:text> ] </xsl:text>
        </xsl:if>

<!-- DOWNLOAD -->
        <xsl:if test="$marc.download">
          <xsl:text>[ </xsl:text><a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$start + position() - 1}+1+R+{$record.syntax}+{$lang}"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_DOWNLOAD']"/></a><xsl:text> ] </xsl:text>
        </xsl:if>
      </xsl:if>

<!-- ORDER -->
      <xsl:if test="$item.order and string-length($user.id) &gt; 0 and @syntax !='1.2.840.10003.5.106' and @syntax !='Extended'">
        <xsl:text>[ </xsl:text><a href="{$cgi.script.URL}?preorder+{$session.id}+1+{$result.set.name}+{$start + position() - 1}+{$record.syntax}+{$lang}"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_PREORDER']"/></a><xsl:text> ] </xsl:text>
      </xsl:if>
...
<!-- RELATED RECORDS -->
  ...
  <xsl:if test="$fmt='M'">
    <xsl:if test="@syntax='1.2.840.10003.5.1' or @syntax='Unimarc' or
			@syntax='1.2.840.10003.5.10' or @syntax='USmarc' or
			@syntax='1.2.840.10003.5.28' or @syntax='RUSmarc'">
      <xsl:text>[ </xsl:text><a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$start + position() - 1}+1+F+{$record.syntax}+{$lang}"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_NOR']"/></a><xsl:text> ]</xsl:text>
      <xsl:text> </xsl:text>
      <xsl:if test="$marc.download">
        <xsl:text>[ </xsl:text><a href="{$cgi.script.URL}?present+{$session.id}+{$result.set.name}+{$start + position() - 1}+1+R+{$record.syntax}+{$lang}"><xsl:value-of select="$msg/messages/localization[@language=$lang]/msg[@id='I_REC_DOWNLOAD']"/></a><xsl:text> ]</xsl:text>
      </xsl:if>
