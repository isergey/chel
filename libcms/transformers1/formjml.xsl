...
<xsl:template name="form.body">
  <xsl:param name="db"/>
  <form name="ZGATE" method="POST" align="right" action="{$cgi.script.URL}" target="_blank">
...
-->
    ...
    <!--  <xsl:value-of select="$msg.form/messages/localization[@lang=$lang]/msg[@id='I_MAXREC']"/>-->
      <input name="MAXRECORDS" type="hidden" value="{$num.records}" size="{$num.records.size}"/>
      <br/>
...
<!-- Records portion size -->
        ...
        <!--<xsl:value-of select="$msg.form/messages/localization[@lang=$lang]/msg[@id='I_MAXREC']"/>-->
        <input name="MAXRECORDS" type="hidden" value="{$num.records}" size="{$num.records.size}" maxlength="{$num.records.size}"/>
        <!--<br/>-->
      <!--  </td>
    <td class="buttons"> -->
...
<xsl:template name="form.ap.list">
  <xsl:param name="number" select="'1'"/>
  <xsl:variable name="x" select="current()"/>
  <!--<input name="USE_{$number}" type="hidden" value="1035" />-->
...
-->
      <xsl:element name="input">
        ...
        <xsl:if test="$resource.reports and explain/configInfo/supports[@type='option' and .='resourceCtrl']">
          <xsl:attribute name="onclick">document.forms.ZGATE.target='_blank'; showProgress('<xsl:value-of select="$cgi.script.URL"/>?report+<xsl:value-of select="$session.id"/>+<xsl:value-of select="$report"/>+<xsl:value-of select="$lang"/>')</xsl:attribute>
        </xsl:if>
      </xsl:element>
      <xsl:choose>
    ...
    <xsl:otherwise>
      <xsl:element name="input">
        ...
        <xsl:if test="$resource.reports and explain/configInfo/supports[@type='option' and .='resourceCtrl']">
  <!--  <xsl:attribute name="onclick">javascript:newWin=window.open('<xsl:value-of select="$cgi.script.URL"/>?report+<xsl:value-of select="$session.id"/>+<xsl:value-of select="$report"/>+<xsl:value-of select="$lang"/>')</xsl:attribute>-->
