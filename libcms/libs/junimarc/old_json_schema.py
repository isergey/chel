...
def make_data_subfield(subfield_dict):
    if subfield_dict.has_key('inner'):
        return make_extended_subfield(subfield_dict)
...
def make_extended_subfield(subfield_dict):
    ...
    for field_dict in subfield_dict.get('inner', {}).get('cf', []):
        fields.append(make_control_field(field_dict))

    for field_dict in subfield_dict.get('inner', {}).get('df', []):
