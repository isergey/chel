...
#        self.osi_longitude = osi_longitude
        self.weight = weight
        self._member_of = []
        self._ldap_object_classes = ['RUSLANorg ']
    def __str__(self):
    ...
    def to_ldap_map(self):
        ...
        if self.o:
            ldap_map[org_attr_map['o']] = self.o.encode('utf-8')

        if self.display_name:
            ldap_map[org_attr_map['display_name']] = self.display_name.encode('utf-8')
...
#            ldap_map[org_attr_map['osi_longitude']] = self.osi_longitude
    ...
    def from_ldap_map(self, ldap_map):
        ...
        for dn_path in ldap.dn.explode_dn(ldap_map[0],ldap.DN_FORMAT_LDAPV3):
            self.dn.append(dn_path.decode('utf-8'))
        self.string_dn = ldap_map[0].decode('utf-8')
        ldap_map = ldap_map[1] #в нулевом элементе содержится dn
        temp = get_ldap_attribute_first(ldap_map, org_attr_map['o'])
        if temp :
            self.o = temp.decode('utf-8')
