...
class LdapWork(object):
    ...
    def get_org_by_attr(self, code=u'', o=u'', display_name=u'',member_of=u'', node=u''):
        """
        return list of users. if no users - return empty []
        """
        o = o.replace('(','\(').replace(')','\)').encode('utf-8')
        filter = []
        if o:
             filter.append('('+ org_attr_map['o'] + '=' + o +')')
        if display_name:
            filter.append('('+ org_attr_map['display_name'] + '=' + display_name.encode('utf-8') +')')
#
#        if district:
#            filter.append('('+ org_attr_map['district'] + '=' + district +')')

        if member_of:
            filter.append('('+ org_attr_map['member_of'] + '=' + member_of.encode('utf-8') +')')

        if code:
            filter.append('('+ org_attr_map['code'] + '=' + code.encode('utf-8') +')')
