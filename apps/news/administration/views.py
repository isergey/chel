...
def news_list(request):
    ...
    if type ==  u'prof':
        news_page = get_page(request, News.objects.filter(prof=True))
    else:
        news_page = get_page(request, News.objects.filter(prof=False))
    ...
    news_list = [news['news'] for news in t_dict.values()]
...
@login_required
@permission_required_or_403('menu.delete_news')
@transaction.commit_on_success
